...
const int NUM_OPCODES = 13;  
const int HALT = 12;  // HALT must be last in the array for end of memory loop to work
const int MAX_REG = 6;
const int REG_BITS = 3;
const int MAX_ADDRESS = 30;
const int MAX_IMMEDIATE = 255;
const int MIN_IMMEDIATE = -256;
const int IMMEDIATE_BITS = 9;
const int IMMEDIATE_MASK = 0x1FF;  // Bits set to mask only immediate of instruction
const int ERROR = -1;
...
const InstructionFormat instructions[NUM_OPCODES] =
{
  ...
  {"MOVI", 10, IMM1},
  {"NOP", 11, REG0},
  {"HALT", 15, REG0}};  // HALT must be last in the array for end of memory loop to work
...
int processImmediateOperands(int *index, Instruction program[], int count, 
  char *ptr )
{
  int num3, immediateValue, num2, num1 = myAtoi(ptr);
...
void writeFile(Instruction program[], int count)
{
  ...
  for(int i = 0; i < count; i++)
  {
    ...
    unsigned num = (program[i].immediateValue & IMMEDIATE_MASK) 
      | (program[i].destReg << IMMEDIATE_BITS )
      | (program[i].sourceRegB << (IMMEDIATE_BITS + REG_BITS))
      | (program[i].sourceRegA<< (IMMEDIATE_BITS + 2 * REG_BITS))
